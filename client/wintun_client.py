...
def set_adapter_ip(ip, prefix_len=24):
    ...
    # Set DNS on all active adapters so nslookup/system resolver uses 8.8.8.8 regardless of interface
    _run_ps(
        'Get-NetAdapter | Where-Object {$_.Status -eq "Up"} | ForEach-Object {'
        f'Set-DnsClientServerAddress -InterfaceIndex $_.ifIndex -ServerAddresses ("{DNS_SERVER}","1.1.1.1") -ErrorAction SilentlyContinue'
        '}'
    )
...
def set_routes(original_gw, if_index):
    ...
    # Disable IPv6 on physical adapters — VPN is IPv4-only, IPv6 would leak
    _run_ps(
        f'Get-NetAdapter | Where-Object {{$_.Status -eq "Up" -and $_.Name -ne "{ADAPTER_NAME}"}} | '
        f'Disable-NetAdapterBinding -ComponentID ms_tcpip6 -ErrorAction SilentlyContinue'
    )
...
def restore_routes(original_gw):
    ...
    _run_ps('Get-NetAdapter | Enable-NetAdapterBinding -ComponentID ms_tcpip6 -ErrorAction SilentlyContinue')
